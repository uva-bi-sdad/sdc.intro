...
income %>% 
  leaflet() %>%
  addProviderTiles(providers$CartoDB.Positron, group = "Base") %>%
  addPolygons(color = ~pal(value), weight = 1, opacity = 1.0, fillOpacity = .8, popup = paste0("Census Tract ", income$geoid, " : $", round(income$value, 1), "average household income"), group = "Income") %>%
  addPolylines(data = counties, opacity = 1, color = "#000000") %>%
  addLegendNumeric(position = "bottomleft", pal = pal, values = ~value, orientation = "horizontal",
...
```{r, results='asis', echo = FALSE, fig.width=5.5, fig.height=4, message = FALSE}
cat('<div width="600px" style="float: right;">')
...
cat('</div>')
```
Using the social impact data commons, we can explore access to additional health services along equity lines. For example, we can explore differences in access to hospitals, primary care physicians, or substance use facilities. We find that access to these services across Fairfax and Arlington do not necessarily follow the same pattern. Using this specific information, policymakers can make more informed decisions regarding closing health equity gaps. 
